...
from pipeline.helpers import cli, db, ffmpeg, sftp, utils
...
def get_file_to_process(
    ...
    WHERE transcript_files.identifier_name is NULL AND
        ...
        aj_path NOT IN (
            ...
            LEFT JOIN transcribeme.wav_conversion
                ON transcribeme.wav_conversion.wc_destination_path =
                    transcribeme.transcribeme_push.transcription_source_path
        )
    ORDER BY aj_name
    LIMIT 1
    """
...
def construct_pending_wav_file_path(
    subject_journals_root: Path, journal_name: str, source_language: str
) -> Path:
    """
    Construct the path for the pending WAV file.

    Files are held here, while waiting for transcripts to be delivered.
    ...
    wav_file_path = (
        ...
        / "pending_audio"
        / f"{journal_name}_{source_language}.wav"
    )
...
if __name__ == "__main__":
    ...
    while True:
        ...
        pending_wav_file_path = construct_pending_wav_file_path(
        ...
        wav_file = File(file_path=pending_wav_file_path)

        # Upload to Transcribeme
        logger.info(
            f"[green]Uploading {pending_wav_file_path} to Transcribeme SFTP server...",
            extra={"markup": True},
        )
        ...
        with Timer() as timer:
            with paramiko.SSHClient() as ssh:
                ...
                with ssh.open_sftp() as sftp_client:
                    sftp_client.chdir("audio")

                    # Check if the file exists
                    file_name = pending_wav_file_path.name
                    file_name = file_name.replace("-", "_")
                    try:
                        sftp_client.stat(file_name)
                        logger.warning(f"File {file_name} exists on SFTP server.")
                        sys.exit(0)
                    except FileNotFoundError:
                        logger.info(f"File {file_name} does not exist on SFTP server.")

                    # Upload the file
                    sftp_upload_path = f"/audio/{file_name}"
                    sftp_client.put(pending_wav_file_path, sftp_upload_path)
                    logger.info(f"File {file_name} uploaded successfully.")

        push_obj = TranscribemePush(
            transcription_source_path=pending_wav_file_path,
            source_language=source_language,
        ...
        insert_query = [wav_file.to_sql(), wav_conversion.to_sql(), push_obj.to_sql()]
        ...
        logger.info(
            f"Inserted {pending_wav_file_path} to database.",
            extra={"markup": True},
        )
