#!/usr/bin/env python
"""
Convert Interview Audio to a standardized (WAV) format.
...
MODULE_NAME = "pipeline.runners.ampscz.qc_interview_audio"
...
def get_file_to_process(
    ...
    This function queries the database for interviews that have not been
    transcribed yet.
    ...
    Returns:
        Optional[Tuple[Path, str, str, str, str]]: A tuple containing the audio path,
            interview type, interview name, subject ID, and study ID.
    ...
    WITH interview_file_counts AS (
        ...
        WHERE public.interview_files.ignored IS FALSE AND
            ...
            public.interviews.interview_type = 'open'
        GROUP BY interview_name
    ),
    available_audio_files AS (
        SELECT
            interview_file AS audio_file,
            i.interview_name,
        ...
        WHERE public.interview_files.ignored IS FALSE AND
            ...
            public.interview_files.interview_file_tags LIKE '%%audio%%' AND
            public.interview_files.interview_file_tags LIKE '%%combined%%' AND
            public.interviews.interview_type = 'open' AND
            i.file_count = 1  -- Only include interviews with exactly one audio file
    ), available_transcript_files AS (
        SELECT public.transcript_files.transcript_file, public.transcript_files.identifier_name AS interview_name
        FROM public.transcript_files
        WHERE public.transcript_files.identifier_type = 'interview'
    )
    SELECT audio_file, interview_name, interview_type, subject_id, study_id
    FROM available_audio_files
    LEFT JOIN available_transcript_files USING (interview_name)
    WHERE available_transcript_files.transcript_file IS NULL AND  -- not yet transcribed
        study_id = '{study_id}' AND
        available_audio_files.audio_file NOT IN (  -- exclude files already in QC or conversion
    ...
    audio_path = Path(result_df.iloc[0]["audio_file"])
    ...
    return audio_path, interview_name, interview_type, subject_id, study_id
...
def construct_temp_wav_file_path(
    subject_interviews_root: Path, interview_name: str, interview_type: str
) -> Path:
    """
    ...
    """
    interview_type_root = subject_interviews_root / interview_type
    wav_file_path = interview_type_root / "temp_audio" / f"{interview_name}.wav"
...
def handle_post_qc(wav_conversion: WavConversion, qc_pass: bool) -> WavConversion:
    ...
    dest_dir = None
    if qc_pass:
        passed_dir = wav_conversion.wc_destination_path.parent.parent / "pending_audio"
        if not passed_dir.exists():
            passed_dir.mkdir(parents=True, exist_ok=True)
        dest_dir = passed_dir
    else:
        failed_dir = wav_conversion.wc_destination_path.parent.parent / "rejected_audio"
...
if __name__ == "__main__":
    ...
    logger.info("Starting audio_qc loop...", extra={"markup": True})
    study_id = studies[0]
    ...
    while True:
        ...
        logger.info(
            f"Converting {audio_path} to {pending_wav_file_path}",
        )
        with Timer() as timer:
            ffmpeg.convert_audio(source=audio_path, target=pending_wav_file_path)

        wav_conversion = WavConversion(
            wc_source_path=audio_path,
            wc_destination_path=pending_wav_file_path,
            wc_duration_s=timer.duration,  # type: ignore
        )

        # QC the audio file
        logger.info(f"Starting QC for {interview_name}")
        with Timer() as timer:
            passed, metrics, fails = qc_audio_file(file_path=pending_wav_file_path)
        if passed:
            logger.info(
                f"Audio QC passed for {interview_name}",
                extra={"markup": True},
            )
        else:
            logger.warning(
                f"Audio QC failed for {interview_name}: {fails}",
                extra={"markup": True},
            )
        ...
        logger.info(
            f"Logged QC results for {interview_name}",
            extra={"markup": True},
        )
